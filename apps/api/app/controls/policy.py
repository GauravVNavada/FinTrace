...
def approval_plan(exception: ExceptionSummary) -> ApprovalPlan:
    if exception.type in {ExceptionType.DUPLICATE_PAYMENT, ExceptionType.AMBIGUOUS_ASSOCIATION} or exception.status in {ExceptionStatus.ESCALATED}:
...
def action_allowed(exception: ExceptionSummary, action: ActionCode) -> bool:
    allowed: dict[ExceptionType, set[ActionCode]] = {
        ...
        ExceptionType.MISSING_SETTLEMENT: {ActionCode.REQUEST_SETTLEMENT_REVIEW, ActionCode.MARK_AS_TIMING_DIFFERENCE},
        ...
        ExceptionType.SETTLEMENT_TIMING: {ActionCode.MARK_AS_TIMING_DIFFERENCE, ActionCode.REQUEST_SETTLEMENT_REVIEW},
        ExceptionType.SETTLEMENT_FEE_VARIANCE: {ActionCode.MARK_AS_EXPECTED_FEE_VARIANCE, ActionCode.REQUEST_SETTLEMENT_REVIEW},
        ExceptionType.MANUAL_WORKFLOW_ANOMALY: {ActionCode.ESCALATE_TO_FINANCE_MANAGER, ActionCode.ESCALATE_TO_CONTROLLER},
    }
    return action in allowed.get(exception.type, {ActionCode.REQUEST_REFUND_REVIEW, ActionCode.ESCALATE_TO_CONTROLLER})

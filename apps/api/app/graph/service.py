...
class LifecycleGraphService:
    ...
    def build(self, organization_id: str, exception_id: str) -> LifecycleGraph:
        ...
        return LifecycleGraph(
            exception_id=exception.id, organization_id=organization_id, nodes=nodes, edges=edges
        )
    ...
    def _nodes_and_edges(
        ...
        def add_node(
            record: dict[str, Any], entity_type: str, id_key: str, label: str | None = None
        ) -> str:
            ...
            nodes.append(
                GraphNode(
                    ...
                    if record.get("amount_minor") is not None
                    else None,
                )
            )
            return record_id
        ...
        for payment in lifecycle.payments:
            ...
            edges.append(
                GraphEdge(source=order_id, target=payment_id, relationship="ORDER_HAS_PAYMENT")
            )
        for settlement in lifecycle.settlements:
            ...
            edges.append(
                GraphEdge(
                    source=payment_id, target=settlement_id, relationship="PAYMENT_HAS_SETTLEMENT"
                )
            )
        for invoice in lifecycle.invoices:
            invoice_id = add_node(invoice, "INVOICE", "invoice_id", "Invoice")
            edges.append(
                GraphEdge(source=order_id, target=invoice_id, relationship="ORDER_HAS_INVOICE")
            )
        for refund in lifecycle.refunds:
            refund_id = add_node(refund, "REFUND", "refund_id", "Refund")
            edges.append(
                GraphEdge(
                    source=str(refund["payment_id"]),
                    target=refund_id,
                    relationship="PAYMENT_HAS_REFUND",
                )
            )
        for movement in lifecycle.inventory_movements:
            ...
            edges.append(
                GraphEdge(
                    source=order_id, target=movement_id, relationship="ORDER_HAS_INVENTORY_MOVEMENT"
                )
            )
        for action in lifecycle.employee_actions:
            ...
            edges.append(
                GraphEdge(
                    source=order_id, target=action_id, relationship="ORDER_HAS_EMPLOYEE_ACTION"
                )
            )
        ...
        if (
            exception_type == ExceptionType.REFUND_WITHOUT_ERP_REVERSAL
            or "ERP_REVERSAL_MISSING" in rules_triggered
        ):
        ...
        for missing_id, entity_type, label in missing:
            nodes.append(
                GraphNode(
                    ...
                    entity_type=entity_type,
                    label=label,
                    state=GraphNodeState.MISSING,
                )
            )
            edges.append(
                GraphEdge(
                    source=order_id, target=missing_id, relationship="ORDER_EXPECTS_MISSING_EVENT"
                )
            )
        return nodes, edges

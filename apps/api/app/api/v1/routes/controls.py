...
def list_audit_events(
    ...
    return [
        AuditEventResponse.model_validate({field: event[field] for field in public_fields})
        for event in events
    ]


@router.post(
    "/exceptions/{exception_id}/resolution-request", response_model=ResolutionRequestResponse
)
def request_resolution(
    ...
    try:
        return controls_service.request_resolution(
            context, exception_id, payload.action_code, idempotency_key
        )
        ...
        raise HTTPException(
            status_code=403, detail={"code": "FORBIDDEN", "message": str(error)}
        ) from error
...
@router.post(
    "/financial-investigations/{investigation_id}/reconciliation-runs/{run_id}/results/{result_id}/resolution-request",
...
def request_financial_resolution(
    ...
    try:
        return controls_service.request_financial_resolution(
            context, investigation_id, run_id, result_id, payload.action_code, idempotency_key
        )
    except ControlNotFoundError as error:
        raise HTTPException(
            status_code=404,
            detail={"code": "RESOURCE_NOT_FOUND", "message": "Uploaded exception does not exist"},
        ) from error
    except ControlForbiddenError as error:
        raise HTTPException(
            status_code=403, detail={"code": "FORBIDDEN", "message": str(error)}
...
def _decide(
    request_id: str, context: ActorContext, idempotency_key: str | None, decision: Decision
) -> ApprovalResponse:
    if idempotency_key is None:
        raise HTTPException(
            status_code=422,
    ...
    try:
        return (
            controls_service.approve(context, request_id, idempotency_key)
            if decision == Decision.APPROVED
            else controls_service.reject(context, request_id, idempotency_key)
        )
    except ControlNotFoundError as error:
        raise HTTPException(
            status_code=404,
            detail={"code": "RESOURCE_NOT_FOUND", "message": "Approval request does not exist"},
        ) from error
    except ControlForbiddenError as error:
        raise HTTPException(
            status_code=403, detail={"code": "FORBIDDEN", "message": str(error)}

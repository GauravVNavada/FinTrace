...
def evaluate_dataset(dataset: Any) -> tuple[EvaluationReport, list[ReconciliationResult]]:
    ...
    matched = [result for result in results if result.status in {"RECONCILED", "RECONCILED_WITH_VARIANCE"}]
    actual_exceptions = [item for item in dataset.ground_truth if item["expected_status"] == "EXCEPTION"]
    detected_exceptions = [result for result in results if result.status == "EXCEPTION"]
    correct_matches = sum(1 for result in matched if truth_by_order[result.order_id]["expected_status"] in {"RECONCILED", "RECONCILED_WITH_VARIANCE"})
    correct_exceptions = sum(1 for result in detected_exceptions if truth_by_order[result.order_id]["expected_status"] == "EXCEPTION")
    return EvaluationReport(
        ...
        unresolved_exceptions=sum(result.status in {"EXCEPTION", "AMBIGUOUS"} for result in results),
    ), results

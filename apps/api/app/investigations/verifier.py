...
def verify_candidate(
    ...
    cited_ids = {item.record_id for item in candidate.supporting_evidence + candidate.contradictory_evidence if item.record_id}
    issues = ["A cited record does not exist in the scoped lifecycle."] if not cited_ids.issubset(valid_ids) else []
...
def _record_ids(lifecycle: CanonicalLifecycle) -> set[str]:
    keys = ("order_id", "payment_id", "settlement_id", "invoice_id", "refund_id", "movement_id", "action_id")
    return {str(value) for rows in (lifecycle.order, *lifecycle.payments, *lifecycle.settlements, *lifecycle.invoices, *lifecycle.refunds, *lifecycle.inventory_movements, *lifecycle.employee_actions) for key in keys if (value := rows.get(key)) is not None}


def _compatibility_issues(candidate: InvestigationCandidate, exception_type: ExceptionType, lifecycle: CanonicalLifecycle) -> list[str]:
    if candidate.root_cause_code is None:
        return ["A root-cause code is required for verification."] if candidate.status == InvestigationStatus.SUPPORTED else []
    expected = {
        ExceptionType.REFUND_WITHOUT_INVENTORY_RETURN: {RootCauseCode.INCOMPLETE_REFUND_WORKFLOW, RootCauseCode.INVENTORY_REVERSAL_MISSING},
        ExceptionType.REFUND_WITHOUT_ERP_REVERSAL: {RootCauseCode.ERP_REVERSAL_MISSING},
        ExceptionType.DUPLICATE_PAYMENT: {RootCauseCode.DUPLICATE_PAYMENT, RootCauseCode.AMBIGUOUS_ASSOCIATION},
    ...
    if candidate.recommended_action_code is not None and not _action_allowed(candidate.recommended_action_code, candidate.root_cause_code):
        return ["Recommendation is not allowed for the selected root-cause code."]
    if candidate.root_cause_code in {RootCauseCode.INCOMPLETE_REFUND_WORKFLOW, RootCauseCode.INVENTORY_REVERSAL_MISSING}:
        has_refund = any(item.source.value == "refund" for item in candidate.supporting_evidence)
        has_missing_return = any(item.source.value == "inventory" and item.record_id is None for item in candidate.supporting_evidence)
        if not has_refund or not has_missing_return:
            return ["Refund workflow conclusions require refund evidence and an explicit missing-return finding."]
    return []
...
def _evidence_score(candidate: InvestigationCandidate, exception_type: ExceptionType, lifecycle: CanonicalLifecycle, issues: list[str]) -> int:
    sources = {item.source.value for item in candidate.supporting_evidence}
    score = 25 if any(item.source.value == "order" and item.record_id for item in candidate.supporting_evidence) else 0
    score += sum({"payment": 15, "settlement": 15, "invoice": 10, "refund": 10, "inventory": 10, "employee_action": 5}.get(source, 0) for source in sources)

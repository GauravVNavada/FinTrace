...
Do not run migrations from application startup. Run `fintrace-migrate` explicitly against the configured database. The runner records applied filenames in `schema_migrations` and applies files in lexical order. These migrations have not been applied on this workstation because PostgreSQL is not installed.

...
export function PatternsPage() {
  ...
  return <>
    ...
    {items.length === 0 ? <Card><CardContent className="py-16 text-center text-sm text-muted-foreground">No recurring patterns meet the minimum two-incident threshold.</CardContent></Card> : <div className="grid gap-4">{items.map(pattern => <Card key={pattern.id}><CardContent className="flex flex-col gap-5 p-5 md:flex-row md:items-center"><div className="flex min-w-0 flex-1 items-start gap-3"><div className="rounded-lg bg-destructive/10 p-2.5 text-destructive"><CircleAlert className="h-5 w-5" /></div><div><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-[10px] text-muted-foreground">{pattern.id}</span><SeverityBadge severity={pattern.severity} /><h3 className="basis-full text-sm font-semibold text-foreground md:basis-auto">{pattern.title}</h3></div><p className="mt-2 max-w-2xl text-xs text-muted-foreground leading-5">{pattern.description}</p></div></div><div className="grid grid-cols-3 gap-6 border-t border-border pt-4 text-xs md:border-l md:border-t-0 md:pl-6 md:pt-0"><div><div className="text-[10px] uppercase tracking-wide text-muted-foreground">Incidents</div><div className="mt-1 font-bold text-foreground">{pattern.incidents}</div></div><div><div className="text-[10px] uppercase tracking-wide text-muted-foreground">Exposure</div><div className="mt-1 font-bold text-foreground">{formatCurrency(pattern.exposure)}</div></div><div><div className="text-[10px] uppercase tracking-wide text-muted-foreground">Location</div><div className="mt-1 font-bold text-foreground">{pattern.location}</div></div></div><div className="rounded-lg bg-muted p-3 md:w-[270px]"><div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wide text-muted-foreground"><ShieldCheck className="h-3.5 w-3.5" />Suggested control</div><p className="mt-1.5 text-[11px] leading-4 text-muted-foreground">{pattern.control}</p><Badge variant="outline" className="mt-3">Signal only</Badge></div></CardContent></Card>)}</div>}
  </>;
}

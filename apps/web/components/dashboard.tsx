...
import { ApiClientError, fetchExceptions, fetchFinancialInvestigationPatterns, fetchFinancialInvestigations, fetchLatestEvaluation, fetchLatestReconciliation, fetchReconciliationResults, launchFlagshipSample, runEvaluation } from "../lib/api-client";
import { downloadCsv } from "../lib/export";
import type { ApiEvaluation, ApiExceptionSummary, ApiFinancialInvestigation, ApiFinancialInvestigationPattern, ApiReconciliationResult, ApiReconciliationRun, ExceptionItem } from "../lib/types";
...
export function Overview() {
    ...
    fetchFinancialInvestigations()
      .then(items => {
        ...
        return Promise.all([
          ...
          if (latestRun) return fetchReconciliationResults(current.id, latestRun.id).then(loadedResults => { if (active) setResults(loadedResults); }).catch(() => undefined);
        });
      })
      .catch(() => { if (active) setUnavailable(true); })
  ...
  async function launchSample() {
    ...
    try {
      const prepared = await launchFlagshipSample(createActionKey("flagship-sample"));
      window.location.href = `/?investigation=${encodeURIComponent(prepared.id)}`;
    } catch (error) {
      setLaunchError(error instanceof ApiClientError ? error.message : "The flagship sample could not be prepared.");
      setLaunching(false);
    }
  }
  ...
  if (!selected) return <><PageHeading eyebrow="Control center" title="Financial integrity workspace" description="Select the investigation you want to inspect. Metrics and patterns never silently switch workspaces."><Button variant="outline" size="sm" onClick={() => void launchSample()} disabled={launching}>{launching ? "Preparing flagship sample…" : "Launch Flagship Sample"}</Button><Button asChild size="sm"><Link href="/investigations/new">Create investigation</Link></Button></PageHeading>{launchError && <Alert variant="destructive" className="mb-4">{launchError}</Alert>}{investigations.length > 1 && <Card className="mb-4"><CardContent className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-sm font-semibold text-foreground">Choose an investigation</div><p className="mt-1 text-xs text-muted-foreground">There are multiple workspaces in this organization.</p></div><Select aria-label="Select investigation" defaultValue="" onChange={event => { if (event.target.value) window.location.href = `/?investigation=${encodeURIComponent(event.target.value)}`; }} className="sm:w-80"><option value="">Select an investigation</option>{investigations.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></CardContent></Card>}{investigations.length <= 1 && <Card><CardContent className="py-16 text-center"><div className="text-sm font-semibold text-foreground">No financial investigation selected</div><p className="mx-auto mt-2 max-w-md text-xs leading-5 text-muted-foreground">Launch the flagship sample for a prepared synthetic lifecycle, or upload source exports yourself.</p><div className="mt-5 flex justify-center gap-2"><Button onClick={() => void launchSample()} disabled={launching}>{launching ? "Preparing…" : "Launch Flagship Sample"}</Button><Button asChild variant="outline"><Link href="/investigations">Open investigations</Link></Button></div></CardContent></Card>}</>;
  const metrics = [
    ["Reconciled", run ? (run.reconciled_count - results.filter(item => item.status === "RECONCILED_WITH_VARIANCE").length).toLocaleString() : "—", run ? "Clean deterministic lifecycles" : "Run the close to calculate"],
    ["Expected variance", run ? results.filter(item => item.status === "RECONCILED_WITH_VARIANCE").length.toLocaleString() : "—", run ? "Timing or fee differences" : "No persisted result"],
    ["Explained findings", run ? run.exception_count.toLocaleString() : "—", run ? "Deterministic explanations available" : "No persisted result"],
    ["Need human decision", run ? run.ambiguous_count.toLocaleString() : "—", run ? "Ambiguous associations" : "No persisted result"],
    ["Potential exposure", run ? formatMinor(run.open_exposure_minor, selected.base_currency) : "—", run ? "Open exposure in this close" : "No persisted result"],
  ];
  return <><PageHeading eyebrow="Month-end close" title={selected.name} description="See what reconciled, what is explained, and what still needs a decision."><Button variant="outline" size="sm" onClick={exportReport} disabled={!run}><Download className="h-3.5 w-3.5" />Export close</Button><Button asChild size="sm"><Link href={`/investigations/${selected.id}`}>{run ? "Continue close" : "Open close"}</Link></Button></PageHeading><div className="mb-6 flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span className="font-mono">{selected.id}</span><span>·</span><span>{selected.base_currency}</span><span>·</span><span>{selected.source_file_count} source files</span><span>·</span><span>{run ? `Run completed ${new Date(run.completed_at ?? run.started_at).toLocaleString()}` : "No run completed"}</span></div><div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{metrics.map(([label, value, detail]) => <Card key={label}><CardContent className="p-5"><div className="text-xs font-medium text-muted-foreground">{label}</div><div className="mt-3 text-[22px] font-bold tracking-tight text-foreground">{value}</div><div className="mt-1 text-[11px] text-muted-foreground">{detail}</div></CardContent></Card>)}</div><div className="grid gap-4 lg:grid-cols-2"><Card><CardHeader><CardTitle>Close progress</CardTitle></CardHeader><CardContent>{run ? <div className="space-y-3 text-xs"><div className="flex items-center justify-between border-b border-border pb-3"><span className="text-muted-foreground">Run status</span><span className="font-semibold text-foreground">{run.status}</span></div><div className="flex items-center justify-between border-b border-border pb-3"><span className="text-muted-foreground">Records accounted for</span><span className="font-semibold text-foreground">{run.records_consumed.toLocaleString()} / {run.records_expected.toLocaleString()}</span></div><div className="flex items-center justify-between"><span className="text-muted-foreground">Dataset</span><span className="font-mono text-[10px] text-foreground">{run.dataset_version_id}</span></div><Button asChild variant="outline" size="sm" className="mt-3"><Link href={`/investigations/${selected.id}#attention`}>View attention queue</Link></Button></div> : <div><p className="text-xs leading-5 text-muted-foreground">This close has no persisted reconciliation run yet. Complete source setup before reconciling the period.</p><Button asChild className="mt-4" size="sm"><Link href={`/investigations/${selected.id}/sources`}>Continue source setup</Link></Button></div>}</CardContent></Card><Card><CardHeader><CardTitle>Recurring patterns</CardTitle></CardHeader><CardContent>{patterns.length === 0 ? <p className="text-xs leading-5 text-muted-foreground">No investigation-scoped patterns meet the minimum occurrence threshold.</p> : <div className="space-y-3">{patterns.slice(0, 4).map(pattern => <div key={pattern.pattern_id} className="rounded-md border border-border p-3 text-xs"><div className="flex items-center justify-between gap-2"><span className="font-semibold text-foreground">{pattern.exception_type}</span><span className="text-muted-foreground">{pattern.occurrence_count} occurrences</span></div><p className="mt-1 leading-5 text-muted-foreground">{pattern.observation}</p><div className="mt-2 font-semibold text-foreground">{formatMinor(pattern.associated_exposure_minor, selected.base_currency)} exposure</div></div>)}<Button asChild variant="link" size="sm" className="px-0"><Link href="/patterns">View patterns</Link></Button></div>}</CardContent></Card></div></>;
}

export function ReviewerSamplePage() {
  return <>
    <PageHeading eyebrow="Workspace" title="FinTrace workspace" description="Review investigation results, controls, patterns, evaluations, and audit." />
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {[
        ["Launch Flagship Sample", "Open the prepared synthetic investigation overview.", "/"],
      ...
      ].map(([title, description, href]) => <Card key={title}><CardContent className="flex h-full flex-col p-5"><div className="text-sm font-semibold text-foreground">{title}</div><p className="mt-2 flex-1 text-xs leading-5 text-muted-foreground">{description}</p><Button asChild variant="outline" size="sm" className="mt-4 w-fit"><Link href={href}>{title === "Launch Flagship Sample" ? "Open overview" : "Open surface"}</Link></Button></CardContent></Card>)}
    </div>
  </>;
}

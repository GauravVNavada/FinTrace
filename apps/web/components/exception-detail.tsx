...
import { loadExceptionDetail } from "../lib/exception-detail-loader";
...
export function ExceptionDetail({ id }: { id: string }) {
  ...
  const [auditAccess, setAuditAccess] = React.useState<"available" | "forbidden" | "unavailable">("available");
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const loaded = await loadExceptionDetail(id, {
          fetchException,
          fetchLifecycle,
          fetchAuditEvents,
          isForbiddenError: error => error instanceof ApiClientError && error.status === 403,
        });
        if (!active) return;
        setException(loaded.exception);
        setLifecycle(loaded.lifecycle);
        setAuditEvents(loaded.auditEvents);
        setAuditAccess(loaded.auditAccess);
      } catch {
...
function AuditCard({ events, access }: { events: ApiAuditEvent[]; access: "available" | "forbidden" | "unavailable" }) { return <Card><CardHeader className="flex-row items-center justify-between"><div><CardTitle>Audit history</CardTitle><p className="mt-1 text-xs text-muted-foreground">Append-only events returned for this exception resource</p></div><ShieldCheck className="h-4 w-4 text-success" /></CardHeader><CardContent className="p-0">{access === "forbidden" ? <p className="px-5 py-6 text-xs text-muted-foreground">Audit history is restricted for your role. The exception and lifecycle evidence remain available.</p> : access === "unavailable" ? <p className="px-5 py-6 text-xs text-muted-foreground">Audit history is temporarily unavailable. The exception and lifecycle evidence remain available.</p> : events.length === 0 ? <p className="px-5 py-6 text-xs text-muted-foreground">No audit events are currently associated with this exception.</p> : <div className="divide-y divide-border">{events.map(event => <div key={event.event_id} className="flex items-center gap-3 px-5 py-3.5"><div className="flex h-7 w-7 items-center justify-center rounded-full bg-muted"><FileText className="h-3.5 w-3.5 text-muted-foreground" /></div><div className="min-w-0 flex-1"><div className="text-xs font-semibold text-foreground">{event.action}</div><div className="mt-0.5 truncate text-[11px] text-muted-foreground">{event.actor_id} · {event.correlation_id}</div></div><span className="font-mono text-[10px] text-muted-foreground">{new Date(event.created_at).toLocaleTimeString()}</span></div>)}</div>}</CardContent></Card>; }

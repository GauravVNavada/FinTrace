...
const canonicalFieldsBySourceType: Record<SourceType, string[]> = {
  ...
  INVENTORY_MOVEMENTS: ["movement_id", "order_id", "sku", "quantity", "movement_type", "occurred_at"],
...
function lifecycleStepState(label: string, record: Record<string, unknown> | undefined, exceptionType: string | null, lifecycle: ApiLifecycleResponse): LifecycleStepState {
  ...
  if (label === "INVENTORY" && exceptionType?.includes("INVENTORY") && !lifecycle.inventory_movements.some(item => item.movement_type === "RETURN")) return "EXPECTED BUT MISSING";
...
function isAiEligibleResult(result: ApiReconciliationResult) {
  ...
  return type.includes("REFUND") || type.includes("INVENTORY_RETURN") || type.includes("AMBIGUOUS");
}
...
export function FinancialInvestigationSourcesPage({ investigationId }: { investigationId: string }) {
  ...
  async function handleFiles(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setNotice(analyzedFiles.length > 0 ? `Uploaded and analyzed ${analyzedFiles.length} source file${analyzedFiles.length === 1 ? "" : "s"}. Review and confirm the proposed mappings before continuing.` : "No source files were analyzed.");
  }

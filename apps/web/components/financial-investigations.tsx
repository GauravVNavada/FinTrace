...
import { analyzeSourceFile, ApiClientError, approveResolution, confirmSourceMappings, createFinancialInvestigation, decideRelationship, deleteSourceFile, discoverRelationships, editSourceMapping, fetchFinancialInvestigation, fetchFinancialInvestigationPatterns, fetchFinancialInvestigations, fetchLatestDataset, fetchLatestReconciliation, fetchProviderHealth, fetchReconciliationResults, fetchReconciliationInvestigation, fetchNormalizedRecords, fetchRelationships, fetchSourceAnalysis, fetchSourceFiles, fetchSourceMappings, fetchLifecycle, generateSampleData, getClientIdentity, investigateReconciliationResult, launchFlagshipSample, normalizeDataset, rejectResolution, requestFinancialResolution, runInvestigationReconciliation, updateSourceClassification, uploadSourceFile } from "../lib/api-client";
import type { ApiFinancialInvestigation, ApiFinancialInvestigationPattern, ApiInvestigation, ApiLifecycleResponse, ApiNormalizedRecord, ApiProviderHealth, ApiReconciliationResult, ApiReconciliationRun, ApiRelationshipProposal, ApiResolutionRequest, ApiSourceAnalysis, ApiSourceFile, ApiSourceMapping, SampleDataRequest, ResolutionActionCode, SourceType } from "../lib/types";
...
export function FinancialInvestigationsPage() {
  ...
  async function launchSample() {
    ...
    try { const prepared = await launchFlagshipSample(requestId()); window.location.href = `/?investigation=${encodeURIComponent(prepared.id)}`; }
  ...
  return <>
    <PageHeading eyebrow="Financial investigations" title="Start with your financial data" description="Create a controlled investigation workspace for source files, mappings, relationships, and reconciliation outcomes.">
      <Button variant="outline" size="sm" onClick={() => void launchSample()} disabled={launching}>{launching ? "Preparing flagship sample…" : "Launch Flagship Sample"}</Button>
      <Button asChild size="sm"><Link href="/investigations/new"><Plus className="h-3.5 w-3.5" />New investigation</Link></Button>
    </PageHeading>
    {launchError && <Alert variant="destructive" className="mb-5"><AlertTitle>Flagship sample could not be prepared</AlertTitle><AlertDescription>{launchError}</AlertDescription></Alert>}
...
export function FinancialInvestigationSourcesPage({ investigationId }: { investigationId: string }) {
  ...
  const [sampleForm, setSampleForm] = React.useState({ orders: 25, seed: 42, anomalyRatePercent: 30 });
  ...
  async function handleSampleGenerate(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (sources.length > 0) return;
    setGenerating(true); setError(null); setNotice(null);
    const payload: SampleDataRequest = { orders: sampleForm.orders, seed: sampleForm.seed, anomaly_rate: sampleForm.anomalyRatePercent / 100 };
    try {
      const result = await generateSampleData(investigationId, payload, requestId());

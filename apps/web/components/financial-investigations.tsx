...
import { analyzeSourceFile, ApiClientError, confirmSourceMappings, createFinancialInvestigation, decideRelationship, deleteSourceFile, discoverRelationships, editSourceMapping, fetchFinancialInvestigation, fetchFinancialInvestigationPatterns, fetchFinancialInvestigations, fetchLatestReconciliation, fetchReconciliationResults, fetchReconciliationInvestigation, fetchRelationships, fetchSourceAnalysis, fetchSourceFiles, fetchSourceMappings, generateDemoData, investigateReconciliationResult, normalizeDataset, requestFinancialResolution, runInvestigationReconciliation, updateSourceClassification, uploadSourceFile } from "../lib/api-client";
import type { ApiFinancialInvestigation, ApiFinancialInvestigationPattern, ApiInvestigation, ApiReconciliationResult, ApiReconciliationRun, ApiRelationshipProposal, ApiResolutionRequest, ApiSourceAnalysis, ApiSourceFile, ApiSourceMapping, DemoDataRequest, ResolutionActionCode, SourceType } from "../lib/types";
...
export function RelationshipReview({ investigationId }: { investigationId: string }) {
  ...
  return <Card className="mt-4"><CardHeader><CardTitle>Relationship review</CardTitle><CardDescription>Only confirmed mappings can produce deterministic join proposals. Accept or reject each proposal before normalization.</CardDescription></CardHeader><CardContent>{error && <Alert variant="destructive" className="mb-3"><AlertDescription>{error}</AlertDescription></Alert>}{loading ? <div role="status" className="text-xs text-muted-foreground">Loading relationship proposals…</div> : items.length === 0 ? <div className="flex flex-col items-start gap-3"><p className="text-xs text-muted-foreground">No proposals yet. Discover relationships after confirming mappings.</p><Button size="sm" onClick={() => void discover()} disabled={working}>Discover relationships</Button></div> : <div className="space-y-3">{items.map(item => <div key={item.id} className="flex flex-col gap-3 rounded-lg border border-border p-4 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><div className="text-xs font-semibold text-foreground">{item.source_file_id} <span className="text-muted-foreground">to</span> {item.target_source_file_id}</div><p className="mt-1 text-[11px] text-muted-foreground">{item.evidence_summary} Join: {item.join_fields.join(", ")} · {Math.round(item.confidence * 100)}%</p></div><div className="flex items-center gap-2"><StatusBadge status={item.status} />{item.status === "PROPOSED" && <><Button variant="outline" size="sm" onClick={() => void decide(item, "REJECTED")} disabled={working}>Reject</Button><Button size="sm" onClick={() => void decide(item, "ACCEPTED")} disabled={working}>Accept</Button></>}</div></div>)}</div>}</CardContent></Card>;
}

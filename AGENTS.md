...
- Keep every reusable UI component in `packages/ui/src/components` and do not put product data there. Keep shared UI infrastructure such as `global.css`, `utils.ts`, and the Tailwind preset directly under `packages/ui/src`.

...
The local product uses synthetic data, deterministic reconciliation, and simulated approvals. Set `AI_PROVIDER=gemini`, `GEMINI_API_KEY_1` (or `AI_API_KEY`), and `AI_MODEL` for live source analysis and iterative exception investigation; if unavailable, the API returns an explicit provider failure and never labels offline output as AI. Gemini receives only bounded headers, inferred types, row count, statistics, and representative sample rows for source analysis, and bounded deterministic findings/evidence for investigation. PostgreSQL workflow state, ordered tool traces, fact-level verifier results, and verified bearer claims are implemented. Uploaded-investigation outcomes are scoped to their financial investigation. Evaluation separates deterministic reconciliation metrics from the independently authored AI investigation benchmark. Staging and production configuration fails closed unless a non-default authentication secret and configured external AI provider are supplied.

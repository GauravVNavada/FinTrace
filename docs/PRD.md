...
# 117. Lifecycle mismatch benchmark (2026-09-05)

The reusable monthly synthetic pack contains one to three anomalous lifecycles per month across settlement, payment, invoice, refund, inventory, and workflow controls. Inventory uses cost basis: `inventory_value = unit_cost * quantity`. A refund amount remains sales-value evidence and is not used as inventory cost.

The benchmark includes missing and incorrect inventory returns, inventory restored without a refund, quantity mismatches, row-level valuation errors, ambiguous payment associations, and the existing financial exceptions. Ground truth remains hidden from ingestion, reconciliation, and investigation providers. Deterministic rules own all money and lifecycle conclusions; live AI explains bounded evidence and may remain unresolved.

...
The supported local release walkthrough is [`docs/walkthrough.md`](walkthrough.md), backed by [`scripts/sample.ps1`](../scripts/sample.ps1). It generates canonical sample artifacts and runs the evaluator with the same seed. Example output is evidence from that local run only; it is not a production accuracy claim. The evaluation API is idempotent and persists public reports when `STORAGE_BACKEND=postgres`; the local sample adapter keeps the same contract in process for isolated tests.

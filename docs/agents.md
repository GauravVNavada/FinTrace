...
The configured provider selects from this allowlist for each exception. The service validates and caps the returned plan before invoking any tool; provider-specific key pools are tried in order only for bounded transient failures, and an explicitly configured fallback provider may continue the same conversation/tool state after `rate_limited`, timeout, temporary-unavailable, or transient-server failures. Authorization, quota exhaustion, unsupported capability, malformed tool arguments, verifier rejection, and invalid FinTrace input surface immediately. The application never falls back to an unconfigured provider. If all configured providers are unavailable, the result is explicitly failed, and if a selected relationship is ambiguous, the result is unresolved. Investigation responses persist the originally requested provider, actual provider/model, fallback flag, and fallback reason.

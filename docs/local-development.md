...
PostgreSQL migrations are defined in `apps/api/migrations/001_initial_schema.sql` through `010_financial_exception_investigations.sql`. Start the database with `docker compose up -d postgres`, then run `fintrace-migrate` and `fintrace-seed` from `apps/api`. The Compose database is published on host port `55432` to avoid colliding with a local PostgreSQL installation; use `postgresql://fintrace:fintrace@localhost:55432/fintrace`. Set `STORAGE_BACKEND=postgres` before starting Uvicorn to exercise durable workflow, source metadata, source analysis, mapping state, reconciliation results, uploaded investigations, and audit state. Application startup must not mutate schema; migrations run explicitly in CI/deployment.
